...
app = webapp2.WSGIApplication(
    [
        ...
        RedirectRoute(
            "/redirect-to-name-destination",
            name="redirect-to-name-destination",
            handler=HomeHandler,
        ),
        RedirectRoute(
            "/redirect-to-name", redirect_to_name="redirect-to-name-destination"
        ),
    ]
)
...
class TestRedirectRoute(BaseTestCase):
    ...
    def test_easy_redirect_to(self):
        ...
        self.assertEqual(
            rsp.headers["Location"], "http://localhost/i-was-redirected-easily"
        )
    ...
    def test_redirect_to_name(self):
        ...
        self.assertEqual(rsp.body, b"")
        self.assertEqual(
            rsp.headers["Location"], "http://localhost/redirect-to-name-destination"
        )
    ...
    def test_build_only(self):
        self.assertRaises(
            ValueError, RedirectRoute, "/", handler=HomeHandler, build_only=True
        )
...
class TestPrefixRoutes(BaseTestCase):
    def test_simple(self):
        ...
        self.assertEqual(
            router.build(webapp2.Request.blank("/"), "name-a", match[0], match[1]), path
        )
        ...
        self.assertEqual(router.match(webapp2.Request.blank(path))[1:], match)
        self.assertEqual(
            router.build(webapp2.Request.blank("/"), "name-a/b", match[0], match[1]),
            path,
        )
        ...
        self.assertEqual(router.match(webapp2.Request.blank(path))[1:], match)
        self.assertEqual(
            router.build(webapp2.Request.blank("/"), "name-a/c", match[0], match[1]),
            path,
        )
        ...
        self.assertEqual(router.match(webapp2.Request.blank(path))[1:], match)
        self.assertEqual(
            router.build(webapp2.Request.blank("/"), "name-a/d", match[0], match[1]),
            path,
        )
        ...
        self.assertEqual(router.match(webapp2.Request.blank(path))[1:], match)
        self.assertEqual(
            router.build(webapp2.Request.blank("/"), "name-a/d/b", match[0], match[1]),
            path,
        )
        ...
        self.assertEqual(router.match(webapp2.Request.blank(path))[1:], match)
        self.assertEqual(
            router.build(webapp2.Request.blank("/"), "name-a/d/c", match[0], match[1]),
            path,
        )
...
class TestDomainRoute(BaseTestCase):
    def test_simple(self):
        ...
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank("/foo")
        )

        match = router.match(
            webapp2.Request.blank("http://my-subdomain.app-id.appspot.com/foo")
        )
        self.assertEqual(match[1:], ((), {"subdomain": "my-subdomain"}))

        match = router.match(
            webapp2.Request.blank("http://another-subdomain.app-id.appspot.com/foo")
        )
    ...
    def test_guide_examples(self):
        router = webapp2.Router(
            [
                ...
                DomainRoute(
                    r"<subdomain:(?!www\.)[^.]+>.mydomain.com",
                    [
                        webapp2.Route("/path2", "Path2", "path2"),
                    ],
                ),
                DomainRoute(
                    r"<:(app-id\.appspot\.com|www\.mydomain\.com)>",
                    [
        ...
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri1b)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri1c)
        )

        self.assertEqual(
            router.match(webapp2.Request.blank(uri2a))[1:], ((), {"subdomain": "sub"})
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri2b)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri2c)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri2d)
        )

        self.assertEqual(router.match(webapp2.Request.blank(uri3a))[1:], ((), {}))
        self.assertEqual(router.match(webapp2.Request.blank(uri3b))[1:], ((), {}))
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri3c)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri3d)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri3e)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri3f)
        )

        self.assertEqual(
            router.match(webapp2.Request.blank(uri4a))[1:], ((), {"subdomain": "sub"})
        )
        self.assertEqual(
            router.match(webapp2.Request.blank(uri4b))[1:], ((), {"subdomain": "sub"})
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri4c)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri4d)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri4e)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri4f)
        )
        self.assertRaises(
            webapp2.exc.HTTPNotFound, router.match, webapp2.Request.blank(uri4g)
        )
