...
class TestAuth(test_base.BaseTestCase):
    ...
    def test_get_user_by_session(self):
        ...
        # Create a user.
        m = models.User
        success, user = m.create_user(auth_id="auth_id", password_raw="password")
    ...
    def test_get_user_by_password(self):
        ...
        self.assertEqual(rv["user_id"], user_id)
        self.assertEqual(
            session_store.sessions["auth"].session_args["max_age"], 86400 * 7 * 3
        )

        # Now remember=False.
        rv = a.get_user_by_password("auth_id", "password")
        self.assertEqual(rv["user_id"], user_id)
        self.assertEqual(session_store.sessions["auth"].session_args["max_age"], None)
    ...
    def test_validate_password(self):
        ...
        self.assertRaises(
            auth.InvalidPasswordError, s.validate_password, "auth_id", "bar"
        )
        self.assertRaises(
            auth.InvalidAuthIdError, s.validate_password, "auth_id_2", "foo"
        )
