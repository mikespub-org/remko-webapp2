...
class TestXSRFToken(unittest.TestCase):
    def test_verify_timeout(self):
        token = xsrf.XSRFToken("user@example.com", "secret", current_time=1354160000)
        token_string = token.generate_token_string()
        token.verify_token_string(token_string, timeout=10, current_time=1354160010)
        self.assertRaises(
    ...
    def test_verify_no_action(self):
        ...
        self.assertRaises(
            ...
            token.verify_token_string,
            xsrf.XSRFToken(
                "user@example.com", "differentsecret", current_time=1354160000
        ...
        self.assertRaises(
            ...
            token.verify_token_string,
            xsrf.XSRFToken(
                "user@example.com", "secret", current_time=1354160000
            ).generate_token_string("action"),
        )
    ...
    def test_verify_substring(self):
        ...
        token = xsrf.XSRFToken("user@example.com", "secret", current_time=1354160000)
        token_string = token.generate_token_string()
        test_token, test_time = base64.urlsafe_b64decode(token_string).split(b"|")
        test_string = base64.urlsafe_b64encode(b"|".join([test_token[:-1], test_time]))
        self.assertRaises(xsrf.XSRFTokenInvalid, token.verify_token_string, test_string)

    def test_verify_bad_base_64(self):
        token = xsrf.XSRFToken("user@example.com", "secret")
        self.assertRaises(xsrf.XSRFTokenMalformed, token.verify_token_string, "wrong!!")
